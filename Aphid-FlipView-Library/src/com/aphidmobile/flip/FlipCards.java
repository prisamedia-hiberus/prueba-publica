...
public class FlipCards {
  ...
  private static final int STATE_FREEZE = 3;
  private static final int STATE_DEFROST = 4;
  ...
  public synchronized void draw(FlipRenderer renderer, GL10 gl) {
    ...
    switch (state) {
      ...
      case STATE_AUTO_ROTATE: {
        ...
        if (oldAngle < INIT_ANGLE) { //bouncing back after flip backward and over the first page
          Assert.assertTrue(forward);
          if (accumulatedAngle >= INIT_ANGLE) {
            accumulatedAngle = INIT_ANGLE;
            setState(STATE_INIT);
          }
        } else {
        ...
        } //ends of `if (oldAngle < INIT_ANGLE) {} else {}`
...
//        make a snapshot
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (state == STATE_TOUCH) {
            ...
            if (accumulatedAngle > frontCards.getIndex() * 180) {
              accumulatedAngle =
                  Math.min(accumulatedAngle,
                           controller.isOverFlipBottomEnabled() ? (frontCards.getIndex() * 180
                                                             + MAX_TIP_ANGLE)
          ...
          if (accumulatedAngle < INIT_ANGLE) {
            accumulatedAngle =
                Math.max(accumulatedAngle, controller.isOverFlipEnabled() ? -MAX_TIP_ANGLE : INIT_ANGLE);
          }

          int anglePageIndex = getPageIndexFromAngle(accumulatedAngle);

          controller.movetTo(anglePageIndex, accumulatedAngle, false);
      ...
      case MotionEvent.ACTION_UP:
          int anglePageIndex = getPageIndexFromAngle(accumulatedAngle);
          controller.movetTo(anglePageIndex, accumulatedAngle, true);
...
//		if (snapShootNeeded){
		if (AphidLog.ENABLE_DEBUG)
			AphidLog.i("Making a Snapshot");
		frontCards.makeASnapshot(controller.getAnimationBitmapFormat());
		backCards.makeASnapshot(controller.getAnimationBitmapFormat());
//	}
	}
